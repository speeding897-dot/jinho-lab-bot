...
# 수정 대상 폴더 목록 (공기업/사기업 폴더명이 정확해야 합니다)
TARGET_FOLDERS = ["jobs_html", "jobs_private_html"]

# 1. [제거 대상] 문제가 되는 구버전 함수 (채팅창에 값을 넣는 부분)
# 이 문자열이 포함된 구간을 찾아 삭제합니다.
OLD_CODE_SIGNATURE = "function askAiAboutNews(title, date) {"

# 2. [교체 대상] 완벽하게 봉인된 신규 함수 (지시사항 노출 X, 서버 직송)
# 자바스크립트 내에서 기업명을 스스로 찾도록(querySelector) 업그레이드했습니다.
NEW_JS_FUNCTION = """
        function askAiAboutNews(title, date) {
            ...
            // [보안 3] 소장님의 영업비밀 지시사항 (절대 노출 안됨)
            // AI에게 합격 DB 활용과 행동 중심 설계를 강조하도록 지시
            ...
            "AI 채용 시대, 합격의 기준은 화려한 문장이 아니라 '검증 가능한 행동 데이터'입니다. 과거 합격 사례를 답습하지 말고, 본인만의 행동 중심 에피소드를 설계하세요. (전문가 첨삭 신청)"`;
            ...
            fetch('https://jinho-lab-bot.onrender.com/chat', {
                method: 'POST',
...
def apply_patch():
    print("🛡️ [보안 패치] 기존 파일들의 지시사항 노출 문제를 수정합니다...")
    ...
    for folder in TARGET_FOLDERS:
            ...
            if filename.endswith(".html"):
                ...
                try:
                    ...
                    if OLD_CODE_SIGNATURE in content:
                        ...
                        # 함수 끝점 찾기 (기존 코드는 sendMsg() 후 닫히는 중괄호들이 있음)
                        # 가장 확실한 방법: 다음 함수인 async function sendMsg() 바로 앞까지 자르기
        ...
        print(f"✅ {folder}: 총 {count}개 파일 수정 완료.")
